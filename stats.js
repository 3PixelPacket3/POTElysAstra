...
let localDb = { creatures: [], rules: [], stats: [], customPresets: {}, activeGroupId: null };
...
// JARVIS UPGRADE: Tactical Timers
window.EAHA_ActiveTimers = {};
window.EAHA_StartTimer = (elementId, seconds) => {
...
const setView = (creature) => {
  ...
  } else elements.image.style.display = 'none';

  const baseStats = creature.stats?.base || {};
  const statLabels = { health: 'Health', combatWeight: 'Combat Weight', armor: 'Armor', carryCapacity: 'Capacity', stamina: 'Stamina', speed: 'Speed' };
...
const saveStats = async () => {
  ...
  creature.stats.base = {
    health: elements.editHealth.value.trim(), combatWeight: elements.editWeight.value.trim(),
    armor: elements.editArmor.value.trim(), carryCapacity: elements.editCarry.value.trim(),
    stamina: elements.editStamina.value.trim(), speed: elements.editSpeed.value.trim()
  };
...
const applyCalculation = () => {
  ...
  if (baseValues.every(v => isNaN(v) || v === 0)) return showToast(`Base ${targetKey} array is empty or invalid.`, 'error');
...
const populateDropdowns = () => {
  ...
  elements.fighterA.innerHTML = ''; elements.fighterB.innerHTML = '';
  elements.fighterA.appendChild(defaultOptionA); elements.fighterB.appendChild(defaultOptionB);
...
const applyModifiersToStats = (baseStatMap, role, mutation, geneticsArray, elderStage, rebirthStage) => {
  ...
  (geneticsArray || []).forEach(gen => {
    if (gen !== 'None') {
      ...
      genWeight += gObj.weight || 0; genHealth += gObj.health || 0; genStamina += gObj.stamina || 0;
      genSpeed += gObj.speed || 0; genArmor += gObj.armor || 0; genTurn += gObj.turn || 0;
    }
  });
  ...
  return {
    weight: Math.max(1, finalWeight), health: Math.max(1, finalHealth), stam: Math.max(1, finalStamina),
    speed: Math.max(0.1, finalSpeed), armor: Math.max(0.01, finalArmor), turn: Math.max(0.1, finalTurn)
  };
};
  ...
  customStats.forEach(stat => {
    ...
    if (lowerName.includes('damage') && !lowerName.includes('debuff') && !lowerName.includes('multiplier') && !lowerName.includes('buff')) {
      ...
      if (baseDmg > 0) {
        ...
        attacks.push({ name: displayName, baseDmg, actualDmg: actualDmg.toFixed(1), cd, dps });
      }
    }
  });
...
const broadcastVitals = (finalA) => {
    ...
    broadcastTimeout = setTimeout(async () => {
        ...
        const payload = {
            ...
            elder: elders[parseInt(elements.fighterAElder.value, 10)] || "Pre-Elder",
            role: elements.fighterARole.value,
            mutation: elements.fighterAMutation.value,
        ...
        } catch(e) { console.error("Telemetry Broadcast failed", e); }
    }, 1500); 
};
...
const runSimulation = () => {
  ...
  if (idA === 'none' || idB === 'none') {
    ...
    elements.simOutputA.innerHTML = ''; elements.simOutputB.innerHTML = '';
    elements.speedComparison.innerHTML = ''; elements.staminaComparison.innerHTML = '';
    return;
  }
  ...
  const rawBaseA = {
    weight: getStatArray(dinoA.stats?.base?.combatWeight)[stageAIndex] || 1, health: getStatArray(dinoA.stats?.base?.health)[stageAIndex] || 1,
    stam: getStatArray(dinoA.stats?.base?.stamina)[stageAIndex] || 1, speed: getStatArray(dinoA.stats?.base?.speed)[stageAIndex] || 1,
    armor: getStatArray(dinoA.stats?.base?.armor)[stageAIndex] || 1, turn: getCustomStageStat(dinoA, 'TurnRadiusMultiplier', stageAIndex) || 1
  };
  const rawBaseB = {
    weight: getStatArray(dinoB.stats?.base?.combatWeight)[stageBIndex] || 1, health: getStatArray(dinoB.stats?.base?.health)[stageBIndex] || 1,
    stam: getStatArray(dinoB.stats?.base?.stamina)[stageBIndex] || 1, speed: getStatArray(dinoB.stats?.base?.speed)[stageBIndex] || 1,
    armor: getStatArray(dinoB.stats?.base?.armor)[stageBIndex] || 1, turn: getCustomStageStat(dinoB, 'TurnRadiusMultiplier', stageBIndex) || 1
  };

  const finalA = applyModifiersToStats(rawBaseA, elements.fighterARole.value, elements.fighterAMutation.value, Array.from(elements.fighterAGenetics).map(sel => sel.value), parseInt(elements.fighterAElder.value, 10), elements.fighterARebirth ? parseInt(elements.fighterARebirth.value, 10) : 0);
  const finalB = applyModifiersToStats(rawBaseB, elements.fighterBRole.value, elements.fighterBMutation.value, Array.from(elements.fighterBGenetics).map(sel => sel.value), parseInt(elements.fighterBElder.value, 10), elements.fighterBRebirth ? parseInt(elements.fighterBRebirth.value, 10) : 0);
  ...
  const generateArsenalHtml = (arsenal) => {
    ...
    return arsenal.map(atk => `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px; font-size: 0.9em;">
        ...
        <div><strong style="color: var(--danger); font-size: 1.1em; margin-right: 10px;" title="Estimated Damage Hit">${atk.actualDmg} HP</strong><strong style="color: #10b981; font-size: 1.1em;" title="True DPS">${atk.dps}/s</strong></div>
      </div>
    `).join('');
  };
  ...
  const formatDelta = (base, final, isDecimal=false) => {
    ...
    return `${isDecimal ? final.toFixed(3) : Math.round(final)} <span style="color:${color}; font-size:0.8em;">(${diff > 0 ? '+' : ''}${diff})</span>`;
  };

  elements.simOutputA.innerHTML = `<h3 style="color: var(--primary); margin-bottom: 5px;">${dinoA.name}</h3><div style="margin-bottom: 10px;">${generateBadges(dinoA, stageAIndex)}</div><div style="font-size: 0.95em; line-height: 1.6; display: grid; grid-template-columns: 1fr 1fr; gap: 5px;"><div><strong class="muted">Health:</strong><br>${formatDelta(rawBaseA.health, finalA.health)} HP</div><div><strong class="muted">Weight:</strong><br>${formatDelta(rawBaseA.weight, finalA.weight)}</div><div><strong class="muted">Armor:</strong><br>${formatDelta(rawBaseA.armor, finalA.armor, true)}</div><div><strong class="muted">Speed:</strong><br>${formatDelta(rawBaseA.speed, finalA.speed, true)}</div></div><hr style="border-top: 1px solid var(--border); margin: 15px 0;"><div style="background: color-mix(in srgb, var(--bg) 50%, transparent); padding: 12px; border-radius: 8px; border: 1px solid var(--border);"><div style="color: ${multiplierA >= 1 ? 'var(--success)' : 'var(--danger)'}; font-size: 1.1em; font-weight: bold; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid var(--border);">Inflicts ${Math.round(multiplierA * 100)}% Damage</div><div style="display: flex; justify-content: space-between; font-size: 0.7em; color: var(--muted); text-transform: uppercase; margin-bottom: 5px;"><span>Attack Arsenal</span><span>HIT | DPS</span></div>${generateArsenalHtml(arsenalA)}</div>`;
  elements.simOutputB.innerHTML = `<h3 style="color: var(--danger); margin-bottom: 5px;">${dinoB.name}</h3><div style="margin-bottom: 10px;">${generateBadges(dinoB, stageBIndex)}</div><div style="font-size: 0.95em; line-height: 1.6; display: grid; grid-template-columns: 1fr 1fr; gap: 5px;"><div><strong class="muted">Health:</strong><br>${formatDelta(rawBaseB.health, finalB.health)} HP</div><div><strong class="muted">Weight:</strong><br>${formatDelta(rawBaseB.weight, finalB.weight)}</div><div><strong class="muted">Armor:</strong><br>${formatDelta(rawBaseB.armor, finalB.armor, true)}</div><div><strong class="muted">Speed:</strong><br>${formatDelta(rawBaseB.speed, finalB.speed, true)}</div></div><hr style="border-top: 1px solid var(--border); margin: 15px 0;"><div style="background: color-mix(in srgb, var(--bg) 50%, transparent); padding: 12px; border-radius: 8px; border: 1px solid var(--border);"><div style="color: ${multiplierB >= 1 ? 'var(--success)' : 'var(--danger)'}; font-size: 1.1em; font-weight: bold; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid var(--border);">Inflicts ${Math.round(multiplierB * 100)}% Damage</div><div style="display: flex; justify-content: space-between; font-size: 0.7em; color: var(--muted); text-transform: uppercase; margin-bottom: 5px;"><span>Attack Arsenal</span><span>HIT | DPS</span></div>${generateArsenalHtml(arsenalB)}</div>`;

  let tacticalAdvice = "";
  const bestDpsA = arsenalA.length > 0 ? parseFloat(arsenalA[0].dps) : 0;
  const bestDpsB = arsenalB.length > 0 ? parseFloat(arsenalB[0].dps) : 0;

  if (finalA.weight > finalB.weight && bestDpsA > bestDpsB) tacticalAdvice = `<strong style="color: var(--success);">OVERWHELMING ADVANTAGE:</strong> You possess both the weight and raw DPS advantage. Facetanking is highly favorable.`;
  else if (finalA.weight > finalB.weight && bestDpsA <= bestDpsB) tacticalAdvice = `<strong style="color: var(--info);">WEIGHT ADVANTAGE, BUT BE CAUTIOUS:</strong> You have the mass, but their attack speed/DPS outpaces yours. Do not miss your strikes.`;
  else if (finalA.weight < finalB.weight) tacticalAdvice = `<strong style="color: var(--danger);">LETHAL THREAT:</strong> Opponent out-weighs you. Facetanking will result in death. Rely on hit-and-run tactics.`;
  else tacticalAdvice = `<strong style="color: var(--info);">EVEN MATCHUP:</strong> Combat weights are identical. This fight comes down to raw DPS, armor hides, and player skill.`;

  if (finalB.turn > finalA.turn) tacticalAdvice += `<br><span style="color: var(--danger); font-size: 0.9em; display: block; margin-top: 8px;">⚠️ <strong>AGILITY WARNING:</strong> Target has a tighter Turn Radius multiplier (${finalB.turn.toFixed(2)} vs ${finalA.turn.toFixed(2)}). Do not let them get behind you.</span>`;
  else if (finalA.turn > finalB.turn) tacticalAdvice += `<br><span style="color: var(--success); font-size: 0.9em; display: block; margin-top: 8px;">🎯 <strong>AGILITY ADVANTAGE:</strong> You can turn tighter than the target (${finalA.turn.toFixed(2)} vs ${finalB.turn.toFixed(2)}). Exploit tail-riding maneuvers.</span>`;

  elements.advantageBanner.innerHTML = tacticalAdvice;

  let enduranceHtml = ``;
  if (finalA.speed > finalB.speed) enduranceHtml += `<div style="padding: 10px; border-bottom: 1px solid var(--border);"><strong style="color: var(--success);">Faster Sprint Speed</strong> (${finalA.speed.toFixed(3)} vs ${finalB.speed.toFixed(3)}). You control the engagement spacing.</div>`;
  else if (finalA.speed < finalB.speed) enduranceHtml += `<div style="padding: 10px; border-bottom: 1px solid var(--border);"><strong style="color: var(--danger);">Slower Sprint Speed</strong> (${finalA.speed.toFixed(3)} vs ${finalB.speed.toFixed(3)}). You cannot outrun them if things go bad.</div>`;
  else enduranceHtml += `<div style="padding: 10px; border-bottom: 1px solid var(--border);"><span class="muted">Sprint Speeds are identical (${finalA.speed.toFixed(3)}).</span></div>`;

  enduranceHtml += `<div style="padding: 10px; display: flex; justify-content: space-between; align-items: center;"><div><strong>Your Max Sprint Time:</strong> <span style="color: var(--info);">${sprintTimeA}</span></div><div style="font-size: 1.2em; color: var(--muted);">VS</div><div style="text-align: right;"><strong>Target Max Sprint Time:</strong> <span style="color: var(--info);">${sprintTimeB}</span></div></div>`;
        ...
        snap.forEach(docSnap => {
            ...
            card.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 5px;">
                    <strong style="color: ${isSelf ? 'var(--success)' : 'var(--primary)'}; font-size: 1.1em;">${data.displayName} ${isSelf ? '(You)' : ''}</strong>
...
const updateTierOptions = () => {
  ...
  if (localDb.creatures) localDb.creatures.forEach((c) => { if (c.tier) tiers.add(c.tier); });
  elements.tierFilter.innerHTML = '';
  Array.from(tiers).forEach(tier => elements.tierFilter.appendChild(new Option(tier, tier === 'All Tiers' ? 'all' : tier)));
};
...
const renderList = () => {
  ...
  const filtered = localDb.creatures.filter(c => {
    return c.name.toLowerCase().includes(searchTerm) && (tierFilter === 'all' || c.tier === tierFilter);
  });
  ...
  [...filtered].sort((a,b) => a.name.localeCompare(b.name)).forEach(creature => {
    const item = document.createElement('div');
    item.className = `list-item ${currentCreatureId === creature.id ? 'active' : ''}`;
    item.style.cursor = 'pointer'; item.style.userSelect = 'none';
    item.innerHTML = `<div style="display: flex; flex-direction: column; width: 100%; pointer-events: none;"><strong style="font-size: 1.1em; color: ${currentCreatureId === creature.id ? 'var(--primary)' : 'var(--text)'};">${creature.name}</strong><span style="font-size: 0.85em; color: var(--muted); font-weight: 500;">${creature.tier || 'Unknown'}</span></div>`;
    item.addEventListener('click', () => { currentCreatureId = creature.id; setView(creature); setMode('view'); renderList(); });
...
const init = async () => {
  ...
  elements.modeButtons.forEach(btn => btn.addEventListener('click', () => setMode(btn.dataset.mode)));
  ...
  elements.calcMod.addEventListener('keypress', (e) => { if (e.key === 'Enter') applyCalculation(); });
