...
import { collection, addDoc } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js"; // JARVIS UPGRADE

// --- Global State ---
let db = { creatures: [], rules: [], stats: [], customPresets: {}, encounters: [], activeGroupId: null };
let currentEncounterId = null;

let charts = { outcome: null, cod: null, location: null, hunted: null, myDinos: null };
...
// JARVIS UPGRADE: Live Killfeed Broadcaster
const broadcastKillfeed = async (outcome, opponent, myCreature) => {
    if (!db.activeGroupId || !auth.currentUser) return;
    
    // Only broadcast actual PvP engagements
    if (!outcome.includes('Victory') && !outcome.includes('Defeat (PvP)')) return;

    const type = outcome.includes('Victory') ? 'kill' : 'death';
    const oppName = opponent && opponent !== 'Unknown' ? opponent : 'Unknown Target';

    try {
        await addDoc(collection(firestoreDb, "groups", db.activeGroupId, "killfeed"), {
            type: type,
            player: auth.currentUser.displayName || 'Pack Mate',
            dino: myCreature || 'Unknown',
            opponent: oppName,
            timestamp: Date.now()
        });
    } catch (e) {
        console.error("Killfeed Broadcast Error:", e);
    }
};
...
const populateCreatureDropdown = () => {
  ...
  if (elements.creatureFilter) elements.creatureFilter.innerHTML = '<option value="all">All My Dinosaurs</option>';
  ...
  if (db.creatures && db.creatures.length > 0) {
    ...
    sorted.forEach(c => {
      ...
      if (elements.creatureFilter) elements.creatureFilter.appendChild(new Option(c.name, c.name));
    });
  }
};
...
const syncForm = (encounter) => {
  if (!encounter) {
    ...
    } else elements.myCreature.value = 'Unknown';
...
const saveEncounter = async () => {
  ...
  if (index >= 0) {
      db.encounters[index] = data;
  } else {
      db.encounters.push(data);
      // Only broadcast brand new encounters to avoid spamming the feed on edits
      broadcastKillfeed(data.outcome, data.opponent, data.myCreature);
  }
...
const handleQuickLog = async (outcomeType) => {
  ...
  // Fire off to the Killfeed
  broadcastKillfeed(outcomeType, newLog.opponent, newLog.myCreature);
...
const updateDashboard = () => {
  ...
  if (creatureFilter !== 'all') filteredEncs = safeEncs.filter(e => e.myCreature === creatureFilter);
  ...
  for (const [loc, count] of Object.entries(deathLocations)) {
    if (count > maxDeaths && loc !== 'Unknown' && loc !== 'Logged via Quick Dashboard') {
      maxDeaths = count; deadliestZone = loc;
    }
  }
  ...
  if (ctxOutcome) {
    charts.outcome = new Chart(ctxOutcome, {
      type: 'doughnut',
      data: { labels: ['Victories', 'Defeats', 'Draws'], datasets: [{ data: [kills, deaths, draws], backgroundColor: ['#10b981', '#ef4444', '#3b82f6'], borderWidth: 0, hoverOffset: 4 }] },
  ...
  if (ctxCod) {
    ...
    charts.cod = new Chart(ctxCod, {
      type: 'pie',
      data: { labels: ['PvP', 'Starvation', 'Dehydration', 'Environment'], datasets: [{ data: [pvpDeaths, starveDeaths, thirstDeaths, envDeaths], backgroundColor: ['#ef4444', '#eab308', '#3b82f6', '#8b5cf6'], borderWidth: 0, hoverOffset: 4 }] },
  ...
  if (ctxLocation) {
    const sortedDeathLocs = Object.entries(deathLocations).filter(([loc]) => loc !== 'Unknown' && loc !== 'Logged via Quick Dashboard').sort((a, b) => b[1] - a[1]).slice(0, 5); 
    charts.location = new Chart(ctxLocation, {
      type: 'bar',
      data: { labels: sortedDeathLocs.map(l => l[0]), datasets: [{ label: 'Deaths', data: sortedDeathLocs.map(l => l[1]), backgroundColor: '#ef4444', borderRadius: 4 }] },
  ...
  if (ctxHunted) {
    ...
    filteredEncs.filter(e => e.outcome.includes('Victory')).forEach(e => {
      if (e.opponent && e.opponent !== 'Unknown') huntedSpecies[e.opponent] = (huntedSpecies[e.opponent] || 0) + 1;
    });
    const sortedHunted = Object.entries(huntedSpecies).sort((a, b) => b[1] - a[1]).slice(0, 5); 
    charts.hunted = new Chart(ctxHunted, {
      type: 'bar',
      data: { labels: sortedHunted.map(h => h[0]), datasets: [{ label: 'Kills', data: sortedHunted.map(h => h[1]), backgroundColor: '#10b981', borderRadius: 4 }] },
  ...
  if (ctxMyDinos) {
    const dinoCounts = {};
    safeEncs.forEach(e => { dinoCounts[e.myCreature] = (dinoCounts[e.myCreature] || 0) + 1; });
    const sortedDinos = Object.entries(dinoCounts).filter(([dino]) => dino !== 'Unknown').sort((a, b) => b[1] - a[1]).slice(0, 5);
    charts.myDinos = new Chart(ctxMyDinos, {
      type: 'bar',
      data: { labels: sortedDinos.map(d => d[0]), datasets: [{ label: 'Deployments', data: sortedDinos.map(d => d[1]), backgroundColor: '#8b5cf6', borderRadius: 4 }] },
      options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } }, plugins: { legend: { display: false } } }
    });
  }
};
...
const init = async () => {
  ...
  elements.dashBtn.addEventListener('click', () => { currentEncounterId = null; renderList(); setViewMode('dashboard'); });
  elements.addBtn.addEventListener('click', () => { currentEncounterId = null; syncForm(null); renderList(); setViewMode('form'); });
